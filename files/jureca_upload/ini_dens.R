...
scat_size = 2.5
p_ini = ggplot(dens_plot_ini,aes(rho,mean, color = as.character(t))) + geom_point(size = scat_size) + scale_color_discrete(name = "T/motivation") + xlab(TeX("$b in $m$")) + ylab(TeX("$\\rho$ in $m^{-2}$"))  + 
